...
import { decreaseQuantity, deleteProduct, increaseQuantity } from "@/redux/ShopSlice";
        ...
        {productData.map((item: ProductsProps) => (
          ...
          >
            <div className="flex items-center gap-x-3 w-full md:w-1/3">
              <span onClick={()=>dispatch(deleteProduct(item?._id))} className="text-lg hover:bg-red-600 cursor-pointer duration-200">
                {" "}
                <AiOutlineClose />
              </span>
              <Image
            ...
            <div className="flex items-center justify-start gap-x-3 border-[1px] border-slate-300 py-2 px-4 w-full md:w-auto">
                <p>Quantity</p>
                 <div className="flex items-center text-lg w-20">
